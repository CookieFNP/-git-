...
void loop() {
    ...
    client.publish(mqtt_topic, "first");
    ...
    if (Serial.available() > 0) {
        ...
        client.publish(mqtt_topic, "second");
        //client.publish(mqtt_topic, receivedByte);
        // 输出1表示接收到数据
        //Serial.println("1");
    }

    // 等待一段时间
    delay(1000);  // 每秒发送一次，单位为毫秒
}
        ...
        if (receivedByte == FRAME_HEADER) {
            ...
            if (receivedByte == FRAME_TAIL) {
                ...
                if (validateFrame(frameBuffer, frameIndex)) {
                    Serial.println("Valid frame received. Parsing data...");
                    parseModbusFrame(frameBuffer, frameIndex);
                } else {
